...
4. Give CLEAR and CONCISE educational answers when context matches the question (aim for 5–7 sentences or 3–6 short bullets)
5. For conceptual questions: Provide a somewhat detailed explanation focused on key ideas; use bullets if helpful
6. For Math/Physics/Chemistry problems: Show a minimal step-by-step solution with necessary calculations only
7. Use simple language appropriate for the student's class level
8. Include relevant examples, diagrams descriptions, or illustrations when helpful
9. If question is outside NCERT curriculum OR context is irrelevant, politely decline
...
Answer:"""
            ...
            try:
                # Generate answer with more detail for comprehensive responses
                max_tokens = 380 if use_step_by_step else 280  # Concise yet useful
        ...
        try:
            ...
            try:
                ...
                response_stream = self.model(
                    final_prompt,
                    max_tokens=600,  # Increased further to ensure complete solutions
